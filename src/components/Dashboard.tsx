...
export default function Dashboard() {
  ...
  const ageGenderData = Object.entries(stats.ageGroupCounts).map(([age, count]) => ({
  ...
  const genderData = Object.entries(stats.genderCounts).map(([name, value]) => ({ name, value }));
    ...
    <div className="p-4 overflow-y-auto h-full">
      ...
      <div className="bg-gradient-to-br from-cyan-50 to-cyan-100 rounded-xl p-4 mb-4">
        ...
        <div className="text-xs text-gray-500 mt-1">
          Закладів: {facilities.length} | Створено: {formatNumber(stats.totalCreated)}
        </div>
      </div>
